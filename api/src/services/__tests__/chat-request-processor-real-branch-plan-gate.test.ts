...
const PROVIDER_DISPATCH_SENTINEL = vi.fn(() => {
  throw new Error(
    'PROVIDER_DISPATCH_DETECTED — real-branch gate must NOT reach orchestrationEngine.execute'
  );
});
...
describe('processChatRequest — dryRun=false consensus dynamic-plan gate', () => {
  it(
    'throws APPROVED_PLAN_REQUIRED (422) when EVAL_REQUIRE_APPROVED_PLAN=true and no fingerprint payload',
    async () => {
  ...
  it(
    'throws APPROVED_PLAN_REQUIRED when requirePlanFingerprintMatch=true and no fingerprint payload',
    async () => {
      process.env.ENABLE_CONSENSUS_DYNAMIC_PLAN = 'true';
  ...
  it(
    'throws PLAN_EXECUTION_PARITY_FAILED (409) when the approved fingerprint is invalid',
    async () => {
      process.env.ENABLE_CONSENSUS_DYNAMIC_PLAN = 'true';
      process.env.EVAL_REQUIRE_APPROVED_PLAN = 'true';
      const { processChatRequest } = await import('../chat-request-processor');
  ...
  it(
    'proceeds past the gate when the approved fingerprint matches (orchestrationEngine reached)',
    async () => {
      ...
      const { buildConsensusRoleSpecificCandidatePools } =
        await import('@/core/orchestration/model-selection/role-specific-candidate-pool-builder');
      const { ConsensusPlanDryRunService } =
  ...
  it(
    'does NOT trigger the gate when dryRun=true (parity-check path handles those)',
    async () => {
