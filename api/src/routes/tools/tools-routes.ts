...
const toolErrorResponseSchema = {
  ...
  properties: {
    ...
    error: {
      description:
        'Error message or details (string for tool failures, object for validation errors)',
    },
    metadata: {
      type: 'object',
      additionalProperties: true,
    ...
    statusCode: {
      type: 'number',
      description: 'HTTP status code (Fastify validation error envelope)',
    },
...
async function executeTool<TArgs>(
  ...
  try {
    ...
    context.log.info(
      { toolName, toolCallId, duration, success: result.success },
    ...
    context.log.error(
      { toolName, toolCallId, duration, error: errorMessage },
...
export async function registerToolsRoutes(rootServer: FastifyInstance): Promise<void> {
  ...
  await rootServer.register(async (server: FastifyInstance) => {
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'grep', executeGrepSearchTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          ...
          const include = Array.isArray(body.include)
            ? body.include.filter((i): i is string => typeof i === 'string')
            : undefined;
          const case_sensitive =
            typeof body.case_sensitive === 'boolean' ? body.case_sensitive : false;
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              pattern: {
                type: 'string',
                description: 'Pattern to search for (filename or content)',
              },
              directory: { type: 'string', description: 'Directory to search in' },
              file_type: {
                type: 'string',
                enum: ['name', 'content', 'both'],
                description: 'Search type: name, content, or both',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
      ...
      async (request, reply) =>
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const type =
              file_type === 'name' || file_type === 'content' || file_type === 'both'
    ...
    server.post(
      '/v1/tools/batch-search-replace',
      {
        schema: {
          tags: ['Tools - Batch Operations'],
          summary: 'Batch search and replace',
          description:
            'Perform search and replace operations across multiple files in a single request.',
          security: [{ bearerAuth: [] }, { apiKeyAuth: [] }],
          body: {
            type: 'object',
            required: ['changes'],
        ...
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const validChanges = changes.filter(
              (c): c is { file_path?: string; old_string?: string; new_string?: string } =>
            ...
            return {
              ...
              search:
                validChanges[0] && typeof validChanges[0].old_string === 'string'
                  ? validChanges[0].old_string
                  : '',
              replace:
                validChanges[0] && typeof validChanges[0].new_string === 'string'
                  ? validChanges[0].new_string
                  : '',
            };
          }
        )
    ...
    server.post(
      '/v1/tools/apply-multi-file-changes',
      {
        schema: {
          tags: ['Tools - Batch Operations'],
          summary: 'Apply changes to multiple files',
          description:
            'Apply create, update, or delete operations to multiple files in a single request.',
          security: [{ bearerAuth: [] }, { apiKeyAuth: [] }],
          body: {
            type: 'object',
            required: ['files'],
        ...
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const validFiles = files.filter(
              (f): f is { path?: string; operation?: string; content?: string } =>
            ...
            const changes = validFiles.map((f) => ({
              ...
              operation:
                ...
                (f.operation === 'create' || f.operation === 'update' || f.operation === 'delete')
                  ? f.operation
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              working_directory: {
                type: 'string',
                description: 'Working directory for Git repository',
              },
            },
          },
          response: {
    ...
    server.post(
          ...
          body: {
            ...
            properties: {
              ...
              files: {
                type: 'array',
                items: { type: 'string' },
                description: 'Specific files to commit (if not provided, uses staged files)',
              },
              all: {
                type: 'boolean',
                default: false,
                description: 'Stage all files before committing',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for Git repository',
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_commit', executeGitCommitTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
    ...
    server.post(
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ref: {
                type: 'string',
                description: 'Git reference (commit hash, branch, etc.) to compare against',
              },
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_diff', executeGitDiffTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          const files =
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_push', executeGitPushTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_pull', executeGitPullTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ...
              from: {
                type: 'string',
                description: 'Starting point (branch, commit, or tag). Defaults to current branch.',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for Git repository',
    ...
    server.post(
          ...
          body: {
            ...
            properties: {
              ...
              squash: {
                type: 'boolean',
                default: false,
                description: 'Squash merge (combine commits into one)',
              },
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_merge', executeGitMergeTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'git_rebase', executeGitRebaseTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
          ...
          body: {
            ...
            properties: {
              ...
              resolution: {
                ...
                description:
                  'Resolution strategy: ours (accept current), theirs (accept incoming), or manual (use custom content)',
              },
              manual_content: {
                type: 'string',
                description:
                  'Custom content for manual resolution (required when resolution is "manual")',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for Git repository',
      ...
      async (request, reply) =>
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const resolution =
              resolutionValue === 'ours' ||
              resolutionValue === 'theirs' ||
              resolutionValue === 'manual'
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              query: {
                type: 'string',
                description: 'Search query (text pattern, function name, class name, etc.)',
              },
              path: { type: 'string', default: '.', description: 'Directory path to search in' },
              max_results: {
                type: 'integer',
              ...
              file_pattern: {
                type: 'string',
                description: 'File pattern filter (e.g., "*.ts", "*.js")',
              },
              include_context: {
                type: 'boolean',
                default: false,
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              query: {
                type: 'string',
                description: 'Semantic search query describing what you want to find',
              },
              file_types: {
                type: 'array',
                items: { type: 'string' },
    ...
    server.post(
      '/v1/tools/find-symbol-references',
      {
        schema: {
          tags: ['Tools - Code Analysis'],
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ...
              project_id: {
                type: 'string',
                description: 'Project ID for project-specific analysis',
              },
              branch: { type: 'string', description: 'Git branch to analyze' },
              include_stats: {
                type: 'boolean',
                default: true,
                description: 'Include code statistics (lines, files, etc.)',
              },
              include_symbols: {
                type: 'boolean',
                default: true,
                description: 'Include symbol information (functions, classes, etc.)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ...
              entry_point: {
                type: 'string',
                description: 'Entry point file or module (alternative to file_path)',
              },
              symbol_name: {
                type: 'string',
                description: 'Specific symbol to trace dependencies for',
              },
              depth: {
                type: 'integer',
                minimum: 1,
      ...
      async (request, reply) =>
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            return {
              file_path:
                getOptionalStringProperty(body, 'file_path') ||
                getOptionalStringProperty(body, 'entry_point'),
          ...
          body: {
            ...
            properties: {
              ...
              include_stats: {
                type: 'boolean',
                default: true,
                description: 'Include statistics for each directory',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
          ...
          body: {
            ...
            properties: {
              ...
              start_line: {
                type: 'integer',
                minimum: 1,
                description: 'Starting line number of code block',
              },
              end_line: {
                type: 'integer',
                minimum: 1,
                description: 'Ending line number of code block',
              },
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              file_path: {
                type: 'string',
                description: 'Optional file path to limit rename scope',
              },
          ...
          body: {
            ...
            properties: {
              ...
              start_line: {
                type: 'integer',
                minimum: 1,
                description: 'Line number where the expression is located',
              },
              end_line: {
                type: 'integer',
                minimum: 1,
                description: 'Optional ending line number for multi-line expressions',
              },
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              file_path: {
                type: 'string',
                description: 'Path to the file containing the function call',
              },
    ...
    server.post(
          ...
          body: {
            ...
            properties: {
              ...
              auto_fix: {
                type: 'boolean',
                default: false,
                description: 'Automatically apply fixes (if false, only reports errors)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
      ...
      async (request, reply) =>
        executeTool(request, reply, 'heal_file', executeHealFileTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ...
              path: {
                type: 'string',
                description: 'Path to a directory to check (alternative to file_path)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
        ...
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            return {
              filePath:
                getOptionalStringProperty(body, 'file_path') ||
                getOptionalStringProperty(body, 'path') ||
                '.',
            };
          }
        )
      ...
      async (request, reply) =>
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const framework =
              frameworkValue === 'vitest' ||
              frameworkValue === 'jest' ||
              frameworkValue === 'mocha' ||
              frameworkValue === 'pytest'
                ? (frameworkValue as 'vitest' | 'jest' | 'mocha' | 'pytest')
                : undefined;
            return {
              filePath: getStringProperty(body, 'file_path'),
              framework,
              language: getOptionalStringProperty(body, 'language') as
                'typescript' | 'javascript' | 'python' | undefined,
        ...
        schema: {
          ...
          body: {
            ...
            properties: {
              ...
              analysis_type: {
                type: 'string',
                enum: ['general', 'code', 'diagram', 'ui', 'text'],
      ...
      async (request, reply) =>
        executeTool(
          ...
          (body: ToolRequestBody) => {
            ...
            const analysis_type =
              analysis_type_value === 'general' ||
              analysis_type_value === 'code' ||
              analysis_type_value === 'diagram' ||
              analysis_type_value === 'ui' ||
    ...
    server.post(
      '/v1/tools/extract-code-from-screenshot',
      {
        schema: {
          tags: ['Tools - Multimodal'],
          ...
          body: {
            ...
            properties: {
              ...
              image_url: {
                type: 'string',
                description: 'URL to the screenshot (alternative to image_path)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'check_todo', executeCheckTodoTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          return {
    ...
    server.post(
      ...
      async (request, reply) =>
        executeTool(request, reply, 'list_todos', executeListTodosTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          const statusValue = getOptionalStringProperty(body, 'status');
        ...
        schema: {
          ...
          querystring: {
            ...
            properties: {
              ...
              name: {
                type: 'string',
                description: 'Filter workflows by name (partial match, case-insensitive)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths in workflow steps',
              },
            },
          },
          response: {
          ...
          body: {
            ...
            properties: {
              ...
              steps: {
                ...
                description: 'Array of workflow steps (tool calls with parameters)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
    ...
    server.post(
      '/v1/tools/jina/reader',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina reader execution completed successfully',
              ...toolResponseSchema,
            },
            400: {
              description: 'Bad request (invalid URL or payload)',
              ...toolErrorResponseSchema,
            },
            401: {
    ...
    server.post(
      '/v1/tools/jina/search',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina search execution completed successfully',
              ...toolResponseSchema,
            },
            400: {
              description: 'Bad request (invalid query or payload)',
              ...toolErrorResponseSchema,
            },
            401: {
              description: 'Unauthorized (missing/invalid credentials)',
    ...
    server.post(
      '/v1/tools/jina/embeddings',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina embeddings execution completed successfully',
              ...toolResponseSchema,
            },
    ...
    server.post(
      '/v1/tools/jina/rerank',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina rerank execution completed successfully',
              ...toolResponseSchema,
            },
            400: { description: 'Bad request (invalid payload)', ...toolErrorResponseSchema },
            401: {
    ...
    server.post(
      '/v1/tools/jina/classify',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina classify execution completed successfully',
              ...toolResponseSchema,
            },
            400: { description: 'Bad request (invalid payload)', ...toolErrorResponseSchema },
            401: {
    ...
    server.post(
      '/v1/tools/jina/segment',
      {
        schema: {
          ...
          response: {
            200: {
              description: 'Jina segment execution completed successfully',
              ...toolResponseSchema,
            },
            400: { description: 'Bad request (invalid payload)', ...toolErrorResponseSchema },
            401: {
    ...
    server.post(
      '/v1/tools/jina/deepsearch',
      {
        schema: {
          tags: ['Tools - Web'],
          summary: 'Run deep search via Jina',
          description:
            'Calls Jina DeepSearch chat completions (`deepsearch.jina.ai/v1/chat/completions`).',
          security: [{ bearerAuth: [] }, { apiKeyAuth: [] }],
          body: {
            type: 'object',
            required: ['messages'],
          ...
          response: {
            200: {
              description: 'Jina deepsearch execution completed successfully',
              ...toolResponseSchema,
            },
            400: { description: 'Bad request (invalid payload)', ...toolErrorResponseSchema },
            401: {
    ...
    server.post(
      '/v1/tools/web-search',
      {
        schema: {
          ...
          description:
            'Perform web search using AI-powered search engines. Supports general, code, documentation, and news searches with configurable depth.',
          security: [{ bearerAuth: [] }, { apiKeyAuth: [] }],
          body: {
            type: 'object',
            required: ['query'],
            properties: {
              ...
              search_depth: {
                ...
                enum: ['basic', 'deep'],
                default: 'basic',
                description: 'Search depth: basic (fast) or deep (comprehensive)',
              },
              working_directory: {
                type: 'string',
                description: 'Working directory for relative paths',
      ...
      async (request, reply) =>
        executeTool(request, reply, 'web_search', executeWebSearchTool, (body: ToolRequestBody) => {
          if (typeof body !== 'object' || body === null) {
            throw new Error('Invalid request body');
          }
          const search_type_value = getOptionalStringProperty(body, 'search_type');
          const search_type =
            search_type_value === 'general' ||
            search_type_value === 'code' ||
            search_type_value === 'docs' ||
            search_type_value === 'news'
              ? (search_type_value as 'general' | 'code' | 'docs' | 'news')
              : undefined;
          const search_depth_value = getOptionalStringProperty(body, 'search_depth');
          const search_depth =
            search_depth_value === 'basic' || search_depth_value === 'deep'
