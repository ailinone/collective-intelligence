...
// ============================================================================
// Worker entrypoint — mirrors app.cjs's bootstrap (Google Auth safety net +
// module-alias registration) but loads the queue-runner instead of the HTTP
// server. Needed because dist/workers/queue-runner.js (like every other
// compiled file) still contains unresolved '@/...' requires — tsc-alias only
// rewrites aliases it can resolve to a relative path; app.cjs is what
// actually registers '@' -> dist/* at runtime for the api service. Running
// queue-runner.js directly via `node` (bypassing this bootstrap) fails with
// MODULE_NOT_FOUND on the first '@/...' require it hits (e.g.
// '@/generated/prisma/index.js' from database/client.js).
// ============================================================================
...
const __logDegradedIntercept = (kind, err) => {
  ...
  console.warn(
    '[google-auth-safety-net] Intercepted ' + kind + ' from Google Auth: ' +
      msg.split('\n')[0] +
      ' — worker continues in degraded mode. ' +
...
// Now require the queue-runner entrypoint
require('./workers/queue-runner.js');
