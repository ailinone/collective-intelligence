...
export function classifyNetworkError(input: ClassifierInput): ClassifiedError {
  ...
  if (CONNECT_CODES.has(code)) {
    return {
      category: 'network_connect_error',
      reason: `connect_${code}`,
      isLocalInfra: true,
      isRetryable: true,
    };
  }
  ...
  if (
    ...
    return {
      category: 'provider_quota_error',
      reason: 'quota_msg_match',
      isLocalInfra: false,
      isRetryable: false,
    };
  }
  ...
  if (matches(msg, ['no_eligible_providers', 'no eligible providers'])) {
    return {
      category: 'provider_config_error',
      reason: 'no_eligible_providers_msg_match',
      isLocalInfra: true,
      isRetryable: false,
    };
  }
  ...
  if (matches(msg, ['enotfound', 'getaddrinfo', 'dns lookup'])) {
    return {
      category: 'network_dns_error',
      reason: 'dns_msg_match',
      isLocalInfra: true,
      isRetryable: true,
    };
  }
  if (
    matches(msg, ['econnrefused', 'etimedout', 'connect etimedout', 'socket hang up', 'network'])
  ) {
    return {
      category: 'network_connect_error',
      reason: 'connect_msg_match',
      isLocalInfra: true,
      isRetryable: true,
    };
  }
  ...
  if (status !== undefined) {
    if (status === 401 || status === 403) {
      ...
      if (matches(body, ['insufficient', 'billing', 'quota', 'credit', 'subscription', 'top up'])) {
        return {
          category: 'provider_quota_error',
          reason: `http_${status}_quota_body`,
          isLocalInfra: false,
          isRetryable: false,
        };
      }
    ...
    if (
      status === 404 &&
      matches(body, ['model']) &&
      matches(body, ['unknown', 'not found', 'does not exist', 'no such'])
    ) {
      return {
        category: 'provider_model_error',
        reason: 'http_404_model',
        isLocalInfra: false,
        isRetryable: false,
      };
    }
