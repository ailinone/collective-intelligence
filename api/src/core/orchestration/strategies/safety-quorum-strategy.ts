...
export class SafetyQuorumStrategy extends BaseStrategy {
  getMetadata(): StrategyMetadata {
    return {
      ...
      description:
        'N models independently assess safety via majority vote. Quorum-sensing + majority-rule optimality.',
      minModels: 3,
      maxModels: 5,
  ...
  async execute(request: ChatRequest, context: OrchestrationContext): Promise<OrchestrationResult> {
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error(`Safety quorum timeout after ${TIMEOUT_MS}ms`)),
          TIMEOUT_MS
        )
      ),
    ]);
  ...
  private async executeCore(
    ...
    log.info(
      { quorumSize, models: selected.map((m) => m.id) },
      'Safety quorum: starting parallel assessment'
    );

    // Observer: phase start
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: selected.map((m) => m.name || m.id),
      summary: `Safety quorum: ${quorumSize} voters assessing safety independently.`,
    });
    ...
    const promises = selected.map(async (model) => {
      try {
        ...
        const safetyRequest: ChatRequest = {
          ...request,
          messages: [{ role: 'system', content: safetyPrompt }, ...request.messages],
        };
    ...
    this.emitObserverEvent(context, {
      ...
      round: 1,
      totalRounds: 1,
      summary: `${responses.length} safety assessments received. Classifying votes.`,
    });
    ...
    log.info(
      {
        ...
        verdict: majorityRefused ? 'REFUSED' : 'ALLOWED',
      },
      'Safety quorum: vote complete'
    );

    // Observer: verdict
    this.emitObserverEvent(context, {
      type: 'quality_assessment',
      summary: `Safety quorum verdict: ${majorityRefused ? 'REFUSED' : 'ALLOWED'} (${refusals.length} refusals, ${compliant.length} compliant).`,
    });
  ...
  async *executeStream(
    ...
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: selected.map((m) => m.name || m.id),
      summary: `Safety quorum: ${quorumSize} voters assessing.`,
    });
    ...
    await Promise.allSettled(
      selected.map(async (model) => {
        ...
        if (typeof content === 'string' && content.trim())
          responses.push({ content, response: exec.response });
      })
    );
    ...
    this.emitObserverEvent(context, {
      type: 'quality_assessment',
      summary: `Verdict: ${majorityRefused ? 'REFUSED' : 'ALLOWED'} (${refusals.length} refusals, ${compliant.length} compliant).`,
    });
  ...
  private isRefusal(content: string): boolean {
    return /i (?:can'?t|cannot|won'?t|will not|am unable to)|sorry,?\s+(?:but\s+)?i|(?:inappropriate|unethical|harmful|dangerous|illegal)|against (?:my|the) (?:guidelines|policies)/i.test(
      content
    );
  }
  ...
  private emptyResult(startTime: number, executions: ModelExecution[]): OrchestrationResult {
    return {
      ...
      totalDuration: Date.now() - startTime,
      totalCost: 0,
      metadata: { strategy: 'safety-quorum', error: 'all-failed' },
    };
  }
}
