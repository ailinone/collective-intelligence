...
export class StigmergicRefinementStrategy extends BaseStrategy {
  getMetadata(): StrategyMetadata {
    return {
      ...
      description:
        'Sequential refinement: draft → refine → critique → synthesize. Each model builds on prior work.',
      minModels: 3,
      maxModels: 4,
  ...
  async execute(request: ChatRequest, context: OrchestrationContext): Promise<OrchestrationResult> {
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error(`Stigmergic refinement timeout after ${TIMEOUT_MS}ms`)),
          TIMEOUT_MS
        )
      ),
    ]);
  ...
  private async executeCore(
    ...
    const [synthesizer, critic, refiner, drafter] =
      sorted.length >= 4
        ? sorted
        : [sorted[0], sorted[1], sorted[2], sorted[Math.min(2, sorted.length - 1)]];
    ...
    log.info(
      { drafter: drafter.id, refiner: refiner.id, critic: critic.id, synthesizer: synthesizer.id },
      'Stigmergic refinement: starting'
    );

    // Observer: phase start
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: [drafter, refiner, critic, synthesizer].map((m) => m.name || m.id),
      summary: 'Stigmergic refinement: drafter → refiner → critic → synthesizer pipeline.',
    });

    // Phase 1: Drafter produces initial draft
    const draft = await this.callModel(
      drafter,
      {
        ...request,
        messages: [{ role: 'system', content: PROMPTS.stigmergicDrafter() }, ...request.messages],
      },
      'drafter',
      context,
      executions
    ...
    this.emitObserverEvent(context, {
      ...
      round: 1,
      totalRounds: 4,
      summary: 'Initial draft produced. Refiner improving.',
    });

    // Phase 2: Refiner improves the draft
    const refined = await this.callModel(
      refiner,
      {
        ...request,
        messages: [
          { role: 'system', content: PROMPTS.stigmergicRefiner },
          {
            role: 'user',
    ...
    this.emitObserverEvent(context, {
      ...
      round: 2,
      totalRounds: 4,
      summary: 'Refinement complete. Critic reviewing.',
    });
    ...
    const critique = await this.callModel(
      critic,
      {
        ...request,
        messages: [
          { role: 'system', content: PROMPTS.stigmergicCritic(originalQ, refined) },
          { role: 'user', content: 'Produce the critique.' },
        ],
      },
      'critic',
      context,
    ...
    this.emitObserverEvent(context, {
      ...
      round: 3,
      totalRounds: 4,
      summary: 'Critique complete. Synthesizer producing final version.',
    });
    this.emitObserverEvent(context, {
      type: 'synthesis_start',
      summary: 'Synthesizer merging refined output with critic feedback.',
    });
    ...
    return {
      ...
      strategyUsed: 'stigmergic-refinement',
  ...
  async *executeStream(
    ...
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: [drafter, refiner, critic, synthesizer].map((m) => m.name || m.id),
      summary: 'Stigmergic: drafter → refiner → critic → synthesizer.',
    });
    yield this.progressChunk('Drafting initial response...', 0, 4);
    for (const c of await this.drainObserverChunks(context)) yield c;

    // Phase 1: Draft
    const drafterReq: ChatRequest = {
      ...request,
      messages: [
        { role: 'system' as const, content: PROMPTS.stigmergicDrafter() },
        ...request.messages,
      ],
    };
    ...
    this.emitObserverEvent(context, {
      ...
      round: 1,
      totalRounds: 4,
      summary: 'Draft complete. Refiner improving.',
    });
    yield this.progressChunk('Refining draft...', 1, 4);
    for (const c of await this.drainObserverChunks(context)) yield c;

    // Phase 2: Refine
    const refinerReq: ChatRequest = {
      ...request,
      messages: [
        { role: 'system' as const, content: PROMPTS.stigmergicRefiner },
        {
          role: 'user' as const,
          content: `ORIGINAL:\n${originalQ}\n\nDRAFT:\n${draft}\n\nImprove it.`,
        },
    ...
    const critique = await this.callModel(
      critic,
      {
        ...
        messages: [
          {
            role: 'user',
            content: `Review this response. Identify issues.\n\nORIGINAL:\n${originalQ}\n\nRESPONSE:\n${refined}\n\nProvide feedback.`,
          },
        ],
      },
      'critic',
    ...
    this.emitObserverEvent(context, {
      ...
      round: 3,
      totalRounds: 4,
      summary: 'Critique done. Synthesizing final version.',
    });
    this.emitObserverEvent(context, {
      type: 'synthesis_start',
      summary: 'Synthesizer producing final version.',
    });
    ...
    this.emitObserverEvent(context, {
      ...
      summary: 'Stigmergic refinement complete.',
    });
    for (const c of await this.drainObserverChunks(context)) yield c;
  }
  ...
  private async callModel(
    ...
    try {
      ...
      const reqWithReasoning =
        reasoningEnabled && request.messages[0]?.role === 'system'
          ? {
              ...request,
              messages: [
                {
      ...
      executions.push({
        ...
        request,
        response: this.errorResponse(model),
        cost: 0,
  ...
  private emptyResult(startTime: number, executions: ModelExecution[]): OrchestrationResult {
    return {
      ...
      totalDuration: Date.now() - startTime,
      totalCost: 0,
      metadata: { strategy: 'stigmergic-refinement', error: 'phase-failed' },
    };
  }
}
