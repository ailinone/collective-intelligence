...
export class SwarmExploreStrategy extends BaseStrategy {
  getMetadata(): StrategyMetadata {
    return {
      ...
      description:
        'N models explore N different angles in parallel. Aggregator synthesizes best composite. Swarm intelligence.',
      minModels: 3,
      maxModels: 7,
  ...
  async execute(request: ChatRequest, context: OrchestrationContext): Promise<OrchestrationResult> {
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error(`Swarm explore timeout after ${TIMEOUT_MS}ms`)),
          TIMEOUT_MS
        )
      ),
    ]);
  ...
  private async executeCore(
    ...
    log.info(
      { aggregator: aggregator.id, explorers: explorers.map((m) => m.id), angles: numExplorers },
      'Swarm explore: launching parallel exploration'
    );

    // Observer: phase start
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: explorers.map((m) => m.name || m.id),
      summary: `Swarm exploration: ${explorers.length} explorers, each from a different angle.`,
    });
    ...
    const promises = explorers.map(async (model, i) => {
      ...
      const exploreReq: ChatRequest = {
        ...request,
        messages: [{ role: 'system', content: explorerPrompt }, ...request.messages],
      };
      ...
      if (r.status === 'fulfilled') {
        ...
        if (r.value.content.trim()) {
          explorations.push({
            ...
            model: r.value.model.displayName || r.value.model.id,
          });
        }
      }
    }
    ...
    this.emitObserverEvent(context, {
      ...
      round: 1,
      totalRounds: 1,
      summary: `${explorations.length} explorations complete from angles: ${explorations.map((e) => e.angle.substring(0, 20)).join(', ')}.`,
    });
    this.emitObserverEvent(context, {
      type: 'synthesis_start',
      modelName: aggregator.name || aggregator.id,
      summary: 'Aggregator synthesizing multi-angle explorations.',
    });

    // Phase 2: Aggregator synthesizes all explorations
    const explorationsText = explorations
      .map((e, i) => `### Exploration ${i + 1} (${e.model})\nAngle: ${e.angle}\n\n${e.content}`)
      .join('\n\n---\n\n');
    ...
    const aggReq: ChatRequest = {
      ...request,
      messages: [
        { role: 'system', content: PROMPTS.swarmAggregator(explorations.length) },
        {
          role: 'user',
    ...
    return {
      ...
      metadata: {
        ...
        successfulExplorations: explorations.length,
        angles: explorations.map((e) => e.angle.substring(0, 30)),
  ...
  async *executeStream(
    ...
    this.emitObserverEvent(context, {
      type: 'phase_start',
      models: explorers.map((m) => m.name || m.id),
      summary: `Swarm: ${explorers.length} explorers from different angles.`,
    });
    ...
    const results = await Promise.allSettled(
      explorers.map(async (model, i) => {
        const angle = EXPLORATION_ANGLES[i % EXPLORATION_ANGLES.length];
        const explorerPrompt = this.withReasoningPrompt(
          PROMPTS.swarmExplorer(angle),
          request,
          model
        );
        const exploreReq: ChatRequest = {
    ...
    for (const r of results) {
      if (r.status === 'fulfilled') {
        executions.push(r.value.exec);
        if (typeof r.value.content === 'string' && r.value.content.trim())
          explorations.push({
            angle: r.value.angle,
            content: r.value.content,
            model: r.value.model.displayName || r.value.model.id,
    ...
    this.emitObserverEvent(context, {
      ...
      round: 1,
      totalRounds: 1,
      summary: `${explorations.length} explorations complete.`,
    });
    this.emitObserverEvent(context, {
      type: 'synthesis_start',
      modelName: aggregator.name || aggregator.id,
      summary: 'Aggregator synthesizing.',
    });
    ...
    this.emitObserverEvent(context, {
      ...
      summary: 'Swarm aggregation complete.',
    });
    for (const c of await this.drainObserverChunks(context)) yield c;
  }
