...
describe('01C.1B-J2-C-R6 — fetchArtificialAnalysisLlmModels', () => {
  ...
  it('rejects non-2xx responses and does NOT include the API key in the error', async () => {
    const fetchFn = vi.fn(async () => makeResponse({ error: 'unauthorized' }, { status: 401 }));
  ...
  it('respects custom endpoint and timeoutMs', async () => {
    const fetchFn = vi.fn(async (url: string) => makeResponse({ status: 200, data: [] }));
    ...
    expect(fetchFn.mock.calls[0]![0] as string).toBe(
      'https://example.test/api/v2/data/llms/models'
    );
  });
