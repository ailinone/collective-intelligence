...
describe('probeCredit (mocked fetch)', () => {
  ...
  it('returns has_credits when balance > 0', async () => {
    ...
    const result = await cb.probeCredit!({
      providerId: 'aihubmix',
      apiKey: 'sk-test',
      timeoutMs: 1000,
    });
...
describe('listModels (mocked fetch)', () => {
  ...
  it('parses OAI-compat models response', async () => {
    ...
    const cb = buildProbeCallbacks({
      providerId: 'openai',
      integrationClass: 'native-openai',
      baseUrl: 'https://api.openai.com',
    });
    const models = await cb.listModels!({
      providerId: 'openai',
      apiKey: 'sk-test',
      timeoutMs: 1000,
    });
  ...
  it('throws on HTTP error', async () => {
    ...
    await expect(
      cb.listModels!({ providerId: 'openai', apiKey: 'sk-bad', timeoutMs: 1000 })
  ...
  it('throws on missing data array', async () => {
    ...
    await expect(
      cb.listModels!({ providerId: 'openai', apiKey: 'sk', timeoutMs: 1000 })
    ).rejects.toThrow(/no_data_array/);
  });
  ...
  it('uses custom modelListPath when provided', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.custom.io/api/models', expect.any(Object));
  });
