...
const codingGenerate: BenchmarkTask[] = [
  {
    ...
    difficulty: 'medium',
    prompt:
      'Write a TypeScript debounce function with generics. Signature: debounce<T extends (...args: unknown[]) => void>(fn: T, ms: number): T. Include proper cleanup.',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Correct TypeScript generics, setTimeout/clearTimeout, returns wrapper, handles this context.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Write a Python function binary_search(arr: list[int], target: int) -> int that returns the index of target or -1. Handle empty arrays and single-element arrays.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Write a React custom hook useInterval(callback, delay) that calls callback at delay intervals, handles cleanup on unmount, and pauses when delay is null.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Implement a concurrent-safe LRU cache in Go with Get(key) and Put(key, value) operations. Use sync.Mutex and container/list. Include a capacity limit.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Write a SQL query (PostgreSQL) that returns the top 3 products by revenue per category, with rank, using window functions. Tables: products(id, name, category_id, price), orders(id, product_id, quantity).',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Write a Rust function that reads a JSON config file, parses it into a Config struct, and validates fields. Use proper error handling with custom error types and the ? operator. Config: { port: u16, host: String, database_url: String }.',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Custom error enum, impl From for error conversion, uses ? operator, proper validation.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Write a Node.js Express middleware that: 1) logs request method+path, 2) adds X-Request-Id header from UUID, 3) measures response time and logs it.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Implement a type-safe finite state machine in TypeScript for an order lifecycle: created → confirmed → shipped → delivered, with cancel possible from created/confirmed. Use discriminated unions and ensure invalid transitions are compile-time errors.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Uses discriminated unions or branded types to make invalid transitions impossible at compile time. Covers all valid transitions and prevents invalid ones. Type-safe, no runtime-only checks.',
...
const codingEdit: BenchmarkTask[] = [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'This function is too long. Extract the validation logic into a separate function:\n```python\ndef process_order(order):\n    # Validation\n    if not order.get("items"):\n        raise ValueError("Order must have items")\n    if order["total"] < 0:\n        raise ValueError("Total cannot be negative")\n    if len(order["items"]) > 100:\n        raise ValueError("Too many items")\n    for item in order["items"]:\n        if item["quantity"] < 1:\n            raise ValueError(f"Invalid quantity for {item[\'name\']}")\n        if item["price"] < 0:\n            raise ValueError(f"Invalid price for {item[\'name\']}")\n    # Processing\n    order["status"] = "confirmed"\n    order["confirmed_at"] = datetime.now()\n    db.save(order)\n    notify_warehouse(order)\n    return order\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Extracts validation into validate_order(), keeps process_order clean, preserves all checks.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Add TypeScript types to this JavaScript code. Define interfaces for all data structures:\n```js\nfunction processUsers(users) {\n  return users\n    .filter(u => u.active && u.age >= 18)\n    .map(u => ({\n      fullName: `${u.firstName} ${u.lastName}`,\n      email: u.email,\n      tier: u.purchases > 100 ? "gold" : u.purchases > 50 ? "silver" : "bronze"\n    }))\n    .sort((a, b) => a.fullName.localeCompare(b.fullName));\n}\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Defines User interface, ProcessedUser interface, proper return type, uses string literal union for tier.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'This Prisma code has an N+1 query problem. Fix it:\n```typescript\nasync function getUsersWithOrders() {\n  const users = await prisma.user.findMany();\n  const result = [];\n  for (const user of users) {\n    const orders = await prisma.order.findMany({ where: { userId: user.id } });\n    const totalSpent = orders.reduce((sum, o) => sum + o.amount, 0);\n    result.push({ ...user, orders, totalSpent });\n  }\n  return result;\n}\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Uses Prisma include or a single query with aggregation. Eliminates the loop of queries.',
    checklistItems: [
  ...
  },
  {
    ...
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Uses useState, useEffect for cleanup, useRef for interval, useCallback for handlers.',
    checklistItems: [
...
const codingDebug: BenchmarkTask[] = [
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'This code has a race condition. Find it and fix it:\n```typescript\nlet balance = 100;\nasync function withdraw(amount: number) {\n  if (balance >= amount) {\n    await simulateNetworkDelay();\n    balance -= amount;\n    return { success: true, balance };\n  }\n  return { success: false, balance };\n}\n// Two simultaneous withdrawals of 80\nPromise.all([withdraw(80), withdraw(80)]).then(console.log);\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Identifies TOCTOU race condition, explains interleaving, provides mutex or atomic fix.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'This pagination has a bug. Users report missing items. Find and fix it:\n```python\ndef get_page(items, page, page_size=10):\n    start = page * page_size\n    end = start + page_size - 1\n    return items[start:end]\n```',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      "Review this auth code for security vulnerabilities and fix all issues:\n```js\nconst q = `SELECT * FROM users WHERE username = '${username}' AND password = '${password}'`;\nconst token = jwt.sign({ userId: results[0].id }, 'secret123');\nres.cookie('session', token);\n```",
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Identifies SQL injection, hardcoded secret, missing JWT expiry, plain password comparison, insecure cookie.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'This React component causes an infinite re-render loop. Find the bug:\n```jsx\nfunction UserProfile({ userId }) {\n  const [user, setUser] = useState(null);\n  useEffect(() => {\n    fetch(`/api/users/${userId}`).then(r => r.json()).then(setUser);\n  });\n  return user ? <div>{user.name}</div> : <div>Loading...</div>;\n}\n```',
    evaluationMethod: 'rubric-checklist',
...
const codingReview: BenchmarkTask[] = [
  {
    ...
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Identifies O(n²) in find_duplicates, suggests set. Identifies eager evaluation in process_large_dataset, suggests generator/early termination.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Review the error handling in this Express route:\n```typescript\napp.post("/api/users", async (req, res) => {\n  try {\n    const user = await db.users.create(req.body);\n    const token = generateToken(user);\n    await sendWelcomeEmail(user.email);\n    res.json({ user, token });\n  } catch (e) {\n    res.status(500).json({ error: e.message });\n  }\n});\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Input validation missing, email failure blocks response, error leaks internal details, no specific HTTP status codes.',
    checklistItems: [
  ...
  },
  {
    ...
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Identifies verb-in-URL anti-pattern, wrong HTTP methods, inconsistent naming, missing HATEOAS basics.',
    checklistItems: [
  ...
  },
  {
    ...
    prompt:
      'Review these TypeScript types for correctness and safety:\n```typescript\ninterface ApiResponse {\n  data: any;\n  status: number;\n  error: string;\n}\nfunction fetchUser(id: string | number): Promise<ApiResponse> {\n  return fetch(`/api/users/${id}`).then(r => r.json());\n}\nfunction getUserName(response: ApiResponse): string {\n  return response.data.user.name;\n}\n```',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Identifies any type, unsafe property access chain, missing error handling, error should be optional.',
    checklistItems: [
...
const analysisTechnical: BenchmarkTask[] = [
  {
    ...
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Covers TPS implications, audit comparison, read/write ratio, makes concrete recommendation.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'An e-commerce platform has: Order Service, Inventory Service, Payment Service, Notification Service. A user places an order which needs to: (1) check inventory, (2) process payment, (3) reserve inventory, (4) send confirmation. Compare Saga pattern vs Two-Phase Commit for this flow. Which would you recommend and why?',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Explains both patterns, discusses failure modes, latency implications, consistency guarantees. Makes a reasoned recommendation for Saga with compensating transactions.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 15000,
    maxCostUsd: 0.06,
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'A SaaS platform has 10M users across 5000 organizations. Some organizations have 500K users, most have <100. Design a sharding strategy for the users table. Consider: query patterns (90% queries filter by org_id), cross-org analytics needs, hot-spot prevention.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Proposes tenant-based sharding with consistent hashing, addresses hot-spot from large orgs, discusses cross-shard queries for analytics, mentions rebalancing strategy.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 20000,
    maxCostUsd: 0.08,
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Compare Kafka vs RabbitMQ vs Redis Streams for a real-time analytics pipeline that processes 100K events/sec, needs at-least-once delivery, 7-day retention, and consumer groups. Recommend one.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Design an autoscaling strategy for a REST API on Kubernetes that handles: baseline 100 req/s, spikes to 10K req/s within 30 seconds, cold-start penalty of 15 seconds per pod. Current setup: 5 pods, 2 CPU each. What scaling approach would you use?',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Discusses HPA with custom metrics, pre-warming, KEDA, buffer capacity, mentions cold-start mitigation strategies.',
    strategies: ['single'],
    maxLatencyMs: 15000,
    maxCostUsd: 0.05,
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Explain how the CAP theorem applies to designing a global user session store. The system must support users across US, EU, and Asia. Each user session must be accessible from any region. Session data includes: userId, permissions, preferences, lastActive. What trade-offs would you make?',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Correctly states CAP theorem, identifies partition tolerance as non-negotiable, discusses AP vs CP trade-off for sessions, suggests eventual consistency as likely best fit.',
    strategies: ['single'],
    maxLatencyMs: 12000,
    maxCostUsd: 0.04,
  },
...
const analysisData: BenchmarkTask[] = [
  {
    ...
    difficulty: 'medium',
    prompt:
      'Our API metrics show: p50 latency dropped from 200ms to 50ms, but p99 increased from 500ms to 3000ms. Error rate stayed at 0.1%. This happened after deploying a new caching layer. What are the most likely explanations?',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Cache hits explain p50 drop, cache misses now slower (cache check + miss + backend), bimodal distribution.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Design an A/B test for changing the checkout flow from 3 pages to 1 page. Current conversion rate: 3.2%. Minimum detectable effect: 0.3pp. Daily checkout starts: 50K. Define: sample size, duration, metrics, guardrail metrics, and analysis plan.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Calculates sample size (~500K per arm at 80% power), defines primary metric (conversion rate), secondary metrics, guardrail metrics (revenue per visitor), statistical test, and duration.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 20000,
    maxCostUsd: 0.08,
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'A fraud detection model has these results on 10,000 transactions:\n- True Positives: 45\n- False Positives: 150\n- True Negatives: 9,755\n- False Negatives: 50\n\nCalculate precision, recall, F1, and accuracy. Is this model good for production fraud detection? Why or why not?',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Correct calculations, discusses precision vs recall trade-off for fraud, identifies high FN rate as critical issue.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Monthly active users (MAU) for a B2B SaaS: Jan: 10K, Feb: 11K, Mar: 12K, Apr: 15K, May: 14K, Jun: 22K, Jul: 21K, Aug: 20K, Sep: 19K, Oct: 25K, Nov: 23K, Dec: 18K. Identify patterns, explain the Jun spike, and explain the Dec drop. Suggest what data you would need to confirm your hypotheses.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Identifies general upward trend, Jun spike (possible product launch or marketing), Dec drop (holiday effect for B2B), suggests additional data needed.',
    strategies: ['single'],
    maxLatencyMs: 12000,
    maxCostUsd: 0.04,
  },
...
const analysisText: BenchmarkTask[] = [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Compare GraphQL vs REST for a mobile app with: limited bandwidth, need for nested data (user → orders → items), 20+ screens with different data needs. Which would you recommend and why?',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Discusses over-fetching (REST disadvantage), query flexibility (GraphQL advantage), bandwidth savings, complexity trade-off. Makes concrete recommendation.',
    strategies: ['single'],
    maxLatencyMs: 12000,
    maxCostUsd: 0.04,
  },
...
const factualQA: BenchmarkTask[] = [
  {
    ...
    difficulty: 'easy',
    prompt:
      'What is the difference between HTTP 401 Unauthorized and 403 Forbidden? When should each be used?',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Explain the JavaScript event loop. What is the difference between the microtask queue and the macrotask queue? Give an example showing their execution order.',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Explains call stack, event loop, microtask queue (Promises), macrotask queue (setTimeout).',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Compare the OAuth2 Authorization Code flow vs Implicit flow vs Client Credentials flow. When should each be used? Which is deprecated and why?',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Correctly describes all three flows, when to use each, identifies Implicit as deprecated, explains PKCE.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Explain the difference between Docker bridge, host, and none network modes. When would you use each?',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Compare Raft vs Paxos vs PBFT consensus algorithms. What are the fault tolerance guarantees of each? Which is used in etcd/Kubernetes?',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Correctly describes all three, fault tolerance (f < n/2 for Raft/Paxos, f < n/3 for PBFT), identifies Raft in etcd.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Explain Pick, Omit, Partial, and Required TypeScript utility types with an example for each.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Explain the difference between git merge and git rebase. When should you use each? What are the dangers of rebasing?',
    evaluationMethod: 'rubric-checklist',
...
const creative: BenchmarkTask[] = [
  {
    ...
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Professional API documentation with: endpoint description, parameter table, request example, response example, error code descriptions.',
    strategies: ['single'],
    maxLatencyMs: 12000,
    maxCostUsd: 0.04,
  },
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'Write an outline for a technical blog post titled "Why We Migrated from Microservices Back to a Modular Monolith". The audience is senior engineers. Include section titles, key points per section, and a compelling introduction paragraph.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Structured outline with compelling intro, 4-6 sections, concrete technical points (not vague), contrarian but evidence-based tone.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 12000,
    maxCostUsd: 0.04,
  },
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Write an ADR (Architecture Decision Record) for: choosing PostgreSQL over MongoDB for a new e-commerce platform. Context: team of 8 developers, 3 have MongoDB experience, complex product catalog with variants/pricing/inventory, ACID requirements for orders.',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Follows ADR format (Title, Status, Context, Decision, Consequences). Considers team experience, ACID needs, catalog complexity. Honest about trade-offs.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 15000,
    maxCostUsd: 0.06,
  },
...
const multiStep: BenchmarkTask[] = [
  ...
  },
  {
    ...
    difficulty: 'hard',
    prompt:
      'Plan a zero-downtime migration from a single PostgreSQL instance to a PostgreSQL cluster with read replicas. Current: 1 primary, 500GB data, 2K TPS. Target: 1 primary + 3 read replicas. Include:\n1. Pre-migration checklist\n2. Step-by-step migration plan\n3. Rollback plan for each step\n4. Verification checklist',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Comprehensive plan with clear steps, each step has rollback, verification at each stage, addresses replication lag.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 25000,
    maxCostUsd: 0.12,
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'A product manager asks: "How long to add real-time notifications to our web app?" Current stack: React frontend, Node.js backend, PostgreSQL. No existing real-time infrastructure.\n\nStep 1: List all required components\nStep 2: Estimate effort for each (in developer-days)\nStep 3: Identify dependencies between components\nStep 4: Propose a schedule for a team of 2 developers',
    evaluationMethod: 'rubric-checklist',
    judgeRubric:
      'Lists components (WebSocket server, notification service, frontend integration, database schema, testing), estimates are reasonable, dependencies clear, schedule accounts for parallelism.',
    checklistItems: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    prompt:
      'Step 1: Write a simple health check endpoint in Express (GET /health that returns { status: "ok", uptime: process.uptime() })\nStep 2: Write the JSDoc documentation for it\nStep 3: Write the Dockerfile to containerize this single-file app',
    evaluationMethod: 'rubric-checklist',
...
const reasoning: BenchmarkTask[] = [
  {
    ...
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Systematic analysis of both options across all dimensions. Identifies key trade-off (write latency vs consistency). Makes reasoned recommendation with explicit reasoning chain.',
    strategies: ['single', 'debate'],
    maxLatencyMs: 20000,
    maxCostUsd: 0.1,
  },
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'A startup spends $12K/month on AWS. Breakdown: EC2 $5K (10 m5.xlarge on-demand), RDS $3K (db.r5.2xlarge multi-AZ), S3 $1K (50TB), CloudFront $2K, NAT Gateway $1K. They need to cut to $8K/month without reducing capacity. What would you recommend and why? Show calculations.',
    evaluationMethod: 'rubric-checklist',
  ...
  },
  {
    ...
    difficulty: 'medium',
    prompt:
      'A healthcare startup wants to use an LLM to pre-screen patient symptoms before doctor appointments. The LLM would ask questions and suggest urgency levels: routine, urgent, or emergency. What are the ethical considerations? What safeguards are needed? Should they proceed?',
    evaluationMethod: 'llm-judge',
    judgeRubric:
      'Identifies key ethical concerns (misdiagnosis liability, bias in training data, patient trust, informed consent). Proposes concrete safeguards. Makes nuanced recommendation (not just yes/no).',
    strategies: ['single', 'debate'],
    maxLatencyMs: 15000,
    maxCostUsd: 0.06,
  },
...
export function getSuiteStats(): {
  ...
  for (const task of BENCHMARK_SUITE) {
    ...
    byEvaluationMethod[task.evaluationMethod] =
      (byEvaluationMethod[task.evaluationMethod] || 0) + 1;
  }
