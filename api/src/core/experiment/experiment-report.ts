...
export function generateReport(
  ...
  const PROVIDER_ERROR_PATTERNS =
    /402|403|404|429|Insufficient credit|Forbidden|rate limit|exhausted|Model not found/i;

  const frozenResults = results.filter((r) => r.phase === 'frozen');
  const frozenProviderErrors = frozenResults.filter(
    (r) => !r.success && r.responseSummary && PROVIDER_ERROR_PATTERNS.test(r.responseSummary)
  );
  const frozenStrategyErrors = frozenResults.filter(
    (r) => !r.success && (!r.responseSummary || !PROVIDER_ERROR_PATTERNS.test(r.responseSummary))
  );
  ...
  const detailedResults: DetailedResults = {
    overallRanking: overallRanking.map((r) => ({
      ...
      ci95: computeConfidenceInterval(
        successfulResults.filter((s) => getLabel(s) === r.label).map((s) => s.qualityScore!)
      ),
    })),
    rankingByTaskType,
    rankingByComplexity,
...
function computeTradeoffsWithEfficiency(
  results: ExperimentExecutionResult[]
): DetailedResults['tradeoffs'] {
  const groups = groupBy(results, (r) => getLabel(r));
  ...
  for (const [label, items] of Object.entries(groups)) {
    ...
    qualityVsCost.push({
      ...
      avgQuality,
      avgCost,
      qualityPerDollar: computeCostEfficiency(avgQuality, avgCost),
    });
    qualityVsLatency.push({
      label,
      avgQuality,
      avgLatency,
      qualityPerSecond: computeQualityPerSecond(avgQuality, avgLatency),
    });
  }
...
function computeConsistencyAnalysis(
  results: ExperimentExecutionResult[]
): DetailedResults['consistencyAnalysis'] {
  ...
  return {
    byMode,
    mostConsistent: byMode[0]?.mode ?? 'unknown',
    leastConsistent: byMode[byMode.length - 1]?.mode ?? 'unknown',
  };
}

// ─── Composite Metrics by Group ────────────────────────────────────────────

function computeCompositeRegretByGroup(
  results: ExperimentExecutionResult[]
): Record<string, CompositeRegret> {
  const groups = groupBy(results, (r) => getLabel(r));
  const allAvgs = Object.entries(groups).map(([label, items]) => ({
    label,
    quality: mean(items.map((r) => r.qualityScore!)),
    cost: mean(items.map((r) => r.costUsd)),
    latency: mean(items.map((r) => r.latencyMs)),
  }));
...
function computeCompositeEfficiencyByGroup(
  results: ExperimentExecutionResult[]
): Record<string, CompositeEfficiency> {
  const groups = groupBy(results, (r) => getLabel(r));
  const result: Record<string, CompositeEfficiency> = {};
  for (const [label, items] of Object.entries(groups)) {
    result[label] = computeCompositeEfficiency(
      mean(items.map((r) => r.qualityScore!)),
      mean(items.map((r) => r.costUsd)),
      mean(items.map((r) => r.latencyMs))
    );
  }
  return result;
}
...
function identifyLimitations(
  ...
  if (failRate > 0.1)
    limitations.push(`High failure rate: ${(failRate * 100).toFixed(1)}% of executions failed`);
  if (successfulResults.length < 300)
    limitations.push(`Sample size (${successfulResults.length}) below recommended minimum of 300`);

  const modes: ExecutionMode[] = ['single-model', 'collective', 'adaptive'];
  for (const mode of modes) {
    const count = successfulResults.filter((r) => r.executionMode === mode).length;
    if (count < 30)
      limitations.push(`${mode} mode has only ${count} successful executions — low confidence`);
  }

  const taskTypes = new Set(successfulResults.map((r) => r.taskType));
  if (taskTypes.size < 5)
    limitations.push(`Only ${taskTypes.size} task types — limited generalization`);

  if (!(config?.freezeLearningDuringEval ?? true)) {
    limitations.push(
      'CRITICAL: Learning systems were NOT frozen during evaluation — adaptive mode may have improved during measurement'
    );
  }

  limitations.push(
    'Quality scores from LLM-as-judge may have systematic biases toward certain response styles'
  );
...
function buildExecutiveSummary(
  ...
  allResults: ExperimentExecutionResult[],
  successfulResults: ExperimentExecutionResult[],
  ranking: ReturnType<typeof computeOverallRanking>,
  ...
  const singleCost = mean(
    successfulResults.filter((r) => r.executionMode === 'single-model').map((r) => r.costUsd)
  );
  const collectiveCost = mean(
    successfulResults.filter((r) => r.executionMode === 'collective').map((r) => r.costUsd)
  );
  ...
  const adaptiveQuality = mean(
    successfulResults.filter((r) => r.executionMode === 'adaptive').map((r) => r.qualityScore!)
  );
  ...
  return {
    ...
    bestOverallApproach: bestOverall
      ? {
          label: bestOverall.label,
          mode: bestOverall.mode,
          avgQuality: bestOverall.avgQuality,
          evidence: getEvidenceStrength(successfulResults, bestOverall.label),
        }
      : { label: 'unknown', mode: 'single-model', avgQuality: 0, evidence: 'inconclusive' },
    bestByScenario,
    collectiveVsTier1: {
      verdict: h2h?.qualityTTest.significant
        ? qualityDelta > 0
          ? 'Single models outperform'
          : 'Collective outperforms'
    ...
    adaptiveValue: {
      verdict:
        adaptiveCount < 5
          ? 'Insufficient data'
          : `Adaptive avg quality: ${adaptiveQuality.toFixed(4)}`,
...
function buildMethodology(
  ...
  const strategies = [
    ...new Set(results.filter((r) => r.executionMode === 'collective').map((r) => r.strategy)),
  ];
  const suiteCoverage = getSuiteCoverage();

  return {
    modelsCompared: models.map((m) => ({
      id: m,
      displayName: m,
    ...
    adaptiveDescription:
      'Full 5-tier adaptive pipeline: triage → archive → Pareto → bandit → heuristic scoring',
    ...
    phases: {
      warmupExecutions: config?.warmupExecutions ?? 0,
      frozenEvaluation: true,
      learningFrozenDuringMeasurement: config?.freezeLearningDuringEval ?? true,
    },
    statisticalMethods: [
      ...
      'Composite regret (quality+cost+latency)',
...
function buildStatisticalAppendix(
  results: ExperimentExecutionResult[],
  headToHead: HeadToHead[]
): StatisticalAppendix {
  const groups = groupBy(results, (r) => getLabel(r));
  ...
  for (const [label, items] of Object.entries(groups)) {
    ...
    sampleSizes[label] = {
      mode,
      n: items.length,
      nSuccessful: items.filter((r) => r.success).length,
      successRate: items.filter((r) => r.success).length / items.length,
    };
    descriptiveStatsByGroup[label] = {
      quality: computeDescriptiveStats(qualities),
      cost: computeDescriptiveStats(items.map((r) => r.costUsd)),
      latency: computeDescriptiveStats(items.map((r) => r.latencyMs)),
    };
    ...
    outliers[label] = {
      ...
      impact:
        outlierIdx.length > 0
          ? `${outlierIdx.length} outlier(s) detected — may affect mean`
          : 'none',
    };
  }
  ...
  const tTests = headToHead.map((h) => ({
    groupA: h.groupA,
    groupB: h.groupB,
    result: h.qualityTTest,
    interpretation: h.qualityTTest.significant
      ? `Significant difference (p=${h.qualityTTest.pValue.toFixed(4)})`
      : `Not significant (p=${h.qualityTTest.pValue.toFixed(4)})`,
  }));
  const effectSizes = headToHead.map((h) => ({
    ...
    practicalSignificance:
      h.effectSize.category === 'large'
        ? 'Practically significant'
        : h.effectSize.category === 'medium'
  ...
  return {
    sampleSizes,
    descriptiveStatsByGroup,
    confidenceIntervals,
    tTests,
    effectSizes,
    outliers,
    methodNotes: [
      "Welch's t-test used (not Student's) due to unequal group sizes and variances",
      "Cohen's d uses pooled standard deviation",
      'Outliers detected via IQR method (1.5× multiplier)',
      'Confidence intervals use t-distribution for n<30, z-approximation for n≥30',
    ],
  };
}
...
function buildDecisionMemo(
  ...
  const singleQ = mean(
    results.filter((r) => r.executionMode === 'single-model').map((r) => r.qualityScore!)
  );
  const collectiveQ = mean(
    results.filter((r) => r.executionMode === 'collective').map((r) => r.qualityScore!)
  );
  const adaptiveQ = mean(
    results.filter((r) => r.executionMode === 'adaptive').map((r) => r.qualityScore!)
  );
  const singleC = mean(
    results.filter((r) => r.executionMode === 'single-model').map((r) => r.costUsd)
  );
  const collectiveC = mean(
    results.filter((r) => r.executionMode === 'collective').map((r) => r.costUsd)
  );
  const singleL = mean(
    results.filter((r) => r.executionMode === 'single-model').map((r) => r.latencyMs)
  );
  const collectiveL = mean(
    results.filter((r) => r.executionMode === 'collective').map((r) => r.latencyMs)
  );
  ...
  const evidenceStrength: EvidenceStrength =
    h2h?.qualityTTest.significant && h2h.effectSize.category !== 'negligible'
      ? confidence === 'high'
        ? 'strong'
        : 'moderate'
  ...
  return {
    ...
    collectiveBeatsTier1: {
      answer:
        qualityGain > 0.02 && h2h?.qualityTTest.significant
          ? 'yes'
          : qualityGain > 0
            ? 'depends'
            : 'no',
      evidence: evidenceStrength,
      details: `Quality delta: ${qualityGain.toFixed(4)}, cost: ${costMult.toFixed(1)}x, latency: ${latencyMult.toFixed(1)}x`,
    },
    collectiveWinsWhere,
    singleModelWinsWhere: singleWinsWhere,
    adaptiveBeatsBoth: {
      answer: adaptiveQ > singleQ && adaptiveQ > collectiveQ ? 'yes' : 'depends',
      evidence:
        results.filter((r) => r.executionMode === 'adaptive').length >= 30 ? 'moderate' : 'weak',
      details: `Adaptive quality: ${adaptiveQ.toFixed(4)}`,
    },
    collectiveWorthCost: {
      answer:
        qualityGain > 0.05 && costMult < 5
          ? 'yes'
          : qualityGain > 0 && costMult < 10
            ? 'marginal'
            : 'no',
      qualityGain,
      costMultiplier: costMult,
      latencyMultiplier: latencyMult,
    },
    productionRecommendation: {
      defaultMode: 'single-model with best-performing model as default',
      escalationPolicy:
        'Use collective strategies for high-complexity tasks where quality delta justifies cost',
      guardrails: [
        'Set cost ceiling per request',
        'Monitor quality regression via shadow evaluation',
        'Use adaptive mode only after sufficient learning data (1000+ executions)',
        'Separate warm-up from production measurement',
      ],
    },
...
function getLabel(r: ExperimentExecutionResult): string {
  switch (r.executionMode) {
    case 'single-model':
      return r.model ?? 'single-unknown';
    case 'collective':
      return `collective:${r.strategy}`;
    case 'collective-tier1':
      return `collective-tier1:${r.strategy}`;
    case 'single-budget':
      return r.model ?? 'budget-unknown';
    case 'adaptive':
      return 'adaptive:auto';
    default:
      return r.strategy ?? 'unknown';
  }
}
...
function getBestSingleModel(results: ExperimentExecutionResult[]): {
  model: string;
  avgQuality: number;
  evidence: string;
} {
  ...
  return {
    ...
    evidence: `Average quality ${bestQuality.toFixed(4)} across ${bestCount} executions`,
  };
}
...
function getConfidence(
  ...
  const minCount = Math.min(
    results.filter((r) => r.executionMode === modeA).length,
    results.filter((r) => r.executionMode === modeB).length
  );
...
function determineFinalVerdict(
  ...
  if (
    adaptiveQ > singleQ &&
    adaptiveQ > collectiveQ &&
    results.filter((r) => r.executionMode === 'adaptive').length >= 30
  )
...
function describeVerdict(verdict: FinalVerdict, results: ExperimentExecutionResult[]): string {
  switch (verdict) {
    case 'single-model-wins':
      return 'Single Tier 1 models deliver the best quality-adjusted results across most scenarios.';
    case 'collective-wins':
      return 'Collective intelligence strategies deliver measurably better quality, justifying the additional cost in most scenarios.';
    case 'adaptive-wins':
      return 'The adaptive system outperforms both static single-model and collective approaches by dynamically selecting the best strategy per request.';
    case 'depends-on-scenario':
      return 'No single approach dominates. The optimal strategy depends on the specific task type, complexity, and cost/latency constraints.';
    case 'inconclusive':
      return `Insufficient evidence to declare a winner. ${results.length} total executions — more data needed for conclusive results.`;
  }
}
...
function extractKeyFindings(
  ...
  if (h2h) {
    findings.push(
      `Quality delta between single-model and collective: ${h2h.qualityDelta.toFixed(4)} (p=${h2h.qualityTTest.pValue.toFixed(4)}, ${h2h.effectSize.category} effect)`
    );
  }
  const best = getBestSingleModel(results);
  if (best.model !== 'unknown')
    findings.push(`Best single model: ${best.model} (avg quality: ${best.avgQuality.toFixed(4)})`);
  findings.push(
    `Total successful executions: ${results.filter((r) => r.success).length} / ${results.length}`
  );
  return findings;
}

function computeBestByScenario(
  results: ExperimentExecutionResult[]
): ExecutiveSummary['bestByScenario'] {
  ...
  for (const [taskType, items] of Object.entries(byTaskType)) {
    ...
    scenarios.push({
      ...
      winner: bestLabel,
      avgQuality: bestQ,
      evidence: getEvidenceStrength(items, bestLabel),
    });
  }
  return scenarios;
}

function computeCollectiveWinScenarios(
  results: ExperimentExecutionResult[]
): DecisionMemo['collectiveWinsWhere'] {
  ...
  for (const [key, items] of Object.entries(byTaskType)) {
    const singleQ = mean(
      items.filter((r) => r.executionMode === 'single-model').map((r) => r.qualityScore!)
    );
    const collectiveQ = mean(
      items.filter((r) => r.executionMode === 'collective').map((r) => r.qualityScore!)
    );
    const singleC = mean(
      items.filter((r) => r.executionMode === 'single-model').map((r) => r.costUsd)
    );
    const collectiveC = mean(
      items.filter((r) => r.executionMode === 'collective').map((r) => r.costUsd)
    );
    if (collectiveQ > singleQ + 0.02) {
      scenarios.push({
        scenario: key,
        qualityGain: collectiveQ - singleQ,
        costMultiplier: singleC > 0 ? collectiveC / singleC : 0,
        evidenceStrength: items.length >= 20 ? 'moderate' : 'weak',
      });
    }
  }
  return scenarios;
...
function computeSingleWinScenarios(
  results: ExperimentExecutionResult[]
): DecisionMemo['singleModelWinsWhere'] {
  ...
  for (const [key, items] of Object.entries(byTaskType)) {
    ...
    if (singleQ >= collectiveQ - 0.02) {
      scenarios.push({
        scenario: key,
        reason: `Single model quality (${singleQ.toFixed(3)}) matches or exceeds collective (${collectiveQ.toFixed(3)}) at lower cost`,
      });
    }
  }
  return scenarios;
...
function extractProvenClaims(
  ...
  for (const h of headToHead) {
    if (h.qualityTTest.significant && h.effectSize.category !== 'negligible') {
      ...
      proven.push(
        `${winner} outperforms ${h.qualityDelta > 0 ? h.groupB : h.groupA} in quality (p=${h.qualityTTest.pValue.toFixed(4)}, ${h.effectSize.category} effect)`
      );
    }
  }
  return proven.length > 0
    ? proven
    : ['No statistically significant differences with meaningful effect sizes detected'];
}

function extractUnprovenClaims(
  results: ExperimentExecutionResult[],
  headToHead: HeadToHead[]
): string[] {
  ...
  for (const h of headToHead) {
    if (!h.qualityTTest.significant) {
      unproven.push(
        `No significant difference between ${h.groupA} and ${h.groupB} (p=${h.qualityTTest.pValue.toFixed(4)})`
      );
    }
  }
...
function extractContextDependentClaims(_results: ExperimentExecutionResult[]): string[] {
  const claims: string[] = [];
  claims.push(
    'Collective intelligence performance varies by task complexity — may only justify cost for high-complexity tasks'
  );
