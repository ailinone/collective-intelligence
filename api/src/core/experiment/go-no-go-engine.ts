...
export function generateGoNoGoReport(
  ...
  const frozenResults = results.filter(
    (r) => r.phase === 'frozen' && r.success && r.qualityScore !== null
  );
  const confirmationResults = results.filter(
    (r) => r.phase === 'confirmation' && r.success && r.qualityScore !== null
  );
  ...
  const phaseSummary = {
    sanityCheck: {
      executed: results.filter((r) => r.phase === 'sanity-check').length,
      passed: results.filter((r) => r.phase === 'sanity-check' && r.success).length > 0,
    },
...
function evaluateApproach(
  ...
  if (avgQuality >= thresholds.qualityFloor) thresholdsMet.push('quality_floor');
    ...
    thresholdsFailed.push(`quality_floor (${avgQuality.toFixed(3)} < ${thresholds.qualityFloor})`);

  if (successRate >= thresholds.successRateFloor) thresholdsMet.push('success_rate');
  else
    thresholdsFailed.push(
      `success_rate (${(successRate * 100).toFixed(1)}% < ${(thresholds.successRateFloor * 100).toFixed(0)}%)`
    );

  if (consistencyIndex >= thresholds.consistencyFloor) thresholdsMet.push('consistency');
  else
    thresholdsFailed.push(
      `consistency (${consistencyIndex.toFixed(3)} < ${thresholds.consistencyFloor})`
    );
  ...
  switch (profile) {
    case 'max-quality':
      ({ verdict, reason } = evaluateMaxQuality(
        approach,
        avgQuality,
        results,
        thresholds,
      ...
      break;
    case 'low-cost':
      ({ verdict, reason } = evaluateLowCost(
        approach,
        avgQuality,
        avgCost,
        results,
      ...
      break;
    case 'low-latency':
      ({ verdict, reason } = evaluateLowLatency(
        approach,
        avgQuality,
        avgLatency,
        results,
    ...
    case 'high-robustness':
      ...
      break;
    case 'generalist':
      ({ verdict, reason } = evaluateGeneralist(
        approach,
        avgQuality,
        avgCost,
        avgLatency,
...
function evaluateMaxQuality(
  ...
  const labelQualities = allLabels
    .map((l) => ({
      label: l,
  ...
  if (rank === 1) {
    return {
      verdict: 'GO',
      reason: `Highest quality (${avgQuality.toFixed(3)}) — rank #1 of ${allLabels.length}`,
    };
  }
  ...
  if (delta < 0.02) {
    return {
      ...
      reason: `Within 2pp of best (${best.label}: ${best.avgQ.toFixed(3)})`,
    };
  }

  return {
    verdict: 'NO-GO',
    reason: `Rank #${rank}, ${(delta * 100).toFixed(1)}pp below best (${best.label})`,
  };
}
...
function evaluateLowCost(
  ...
  const allEfficiencies = [...new Set(results.map((r) => getLabel(r)))]
    .map((l) => {
      const items = results.filter((r) => getLabel(r) === l);
      return {
        label: l,
        eff: computeCostEfficiency(
  ...
  if (rank <= 2) {
    return {
      verdict: 'GO',
      reason: `Top-2 cost efficiency (${efficiency.toFixed(1)} quality/$ — rank #${rank})`,
    };
  }
  ...
  if (approach.mode === 'collective') {
    ...
    const singleAvgCost = singleCostRows.length
      ? mean(singleCostRows.map((r) => r.costUsd))
      : singles.length
        ? mean(singles.map((r) => r.costUsd))
        : avgCost;
    ...
    for (const r of singles) {
      ...
      if (cur === undefined || r.qualityScore > cur)
        bestSingleByTask.set(r.taskIndex, r.qualityScore);
    }
    ...
    const bestSingleQuality = singles.length
      ? Math.max(...singles.map((r) => r.qualityScore!))
      : avgQuality;
    const qualityGain = pairedGains.length
      ? meanDelta(pairedGains)
      : avgQuality - bestSingleQuality;

    if (avgCost <= singleAvgCost) {
      return {
        verdict: 'GO',
        reason: `Cost ≤ single-model (${costMult.toFixed(2)}x) at quality ${avgQuality.toFixed(3)}`,
      };
    }
    if (costMult > thresholds.maxCostMultiplierForCollective) {
      if (qualityGain >= thresholds.minQualityGainForCollective) {
        return {
          verdict: 'CONDITIONAL-GO',
          reason: `Cost ${costMult.toFixed(1)}x single but quality +${(qualityGain * 100).toFixed(1)}pp over the BEST individual justifies it`,
        };
      }
      return {
        verdict: 'NO-GO',
        reason: `Cost ${costMult.toFixed(1)}x single-model, quality only +${(qualityGain * 100).toFixed(1)}pp vs best individual (need +${(thresholds.minQualityGainForCollective * 100).toFixed(0)}pp)`,
      };
    }
    // Cost is higher but within the acceptable multiplier.
    return {
      verdict: qualityGain >= 0 ? 'GO' : 'CONDITIONAL-GO',
      reason: `Cost ${costMult.toFixed(1)}x single (≤${thresholds.maxCostMultiplierForCollective}x), quality ${qualityGain >= 0 ? '+' : ''}${(qualityGain * 100).toFixed(1)}pp vs best individual`,
    };
  }

  return {
    verdict: 'CONDITIONAL-GO',
    reason: `Moderate cost efficiency (rank #${rank}), acceptable if quality justifies`,
  };
}
...
function evaluateLowLatency(
  ...
  if (
    approach.mode === 'collective' &&
    latencyMultiplier > thresholds.maxLatencyMultiplierForCollective
  ) {
    return {
      verdict: 'NO-GO',
      reason: `Latency ${(avgLatency / 1000).toFixed(1)}s is ${latencyMultiplier.toFixed(1)}x single-model — exceeds ${thresholds.maxLatencyMultiplierForCollective}x limit`,
    };
  }

  const allLatencies = [...new Set(results.map((r) => getLabel(r)))]
    .map((l) => ({
      label: l,
      lat: mean(results.filter((r) => getLabel(r) === l).map((r) => r.latencyMs)),
    }))
  ...
  if (rank <= 3) {
    return {
      verdict: 'GO',
      reason: `Low latency: ${(avgLatency / 1000).toFixed(1)}s (rank #${rank})`,
    };
  }

  return {
    verdict: 'CONDITIONAL-GO',
    reason: `Moderate latency: ${(avgLatency / 1000).toFixed(1)}s (rank #${rank})`,
  };
}
...
function evaluateHighRobustness(
  ...
  if (consistencyIndex >= 0.7) {
    return {
      verdict: 'CONDITIONAL-GO',
      reason: `Moderate consistency (${consistencyIndex.toFixed(3)})`,
    };
  }
...
function evaluateGeneralist(
  ...
  const compositeScore =
    (normalizedQuality * consistencyIndex) /
    (Math.max(normalizedCost, 0.01) * Math.max(normalizedLatency, 0.01));

  // Rank among all approaches
  const allScores = [...new Set(results.map((r) => getLabel(r)))]
    .map((l) => {
      const items = results.filter((r) => getLabel(r) === l);
  ...
  if (rank === 1) {
    return {
      verdict: 'GO',
      reason: `Best generalist score (rank #1, composite: ${compositeScore.toFixed(2)})`,
    };
  }
  if (rank <= 3) {
    return {
      verdict: 'CONDITIONAL-GO',
      reason: `Strong generalist (rank #${rank}, composite: ${compositeScore.toFixed(2)})`,
    };
  }
  ...
  for (const row of rows) {
    ...
    for (const column of columns) {
      const cellResults = results.filter(
        (r) => r.taskType === taskType && r.complexity === complexity && getLabel(r) === column
      );
      if (cellResults.length === 0) continue;
      cells.push({
        row,
        column,
        value: mean(cellResults.map((r) => r.qualityScore!)),
        sampleSize: cellResults.length,
      });
    }
  }
...
function buildConfidenceMap(
  results: ExperimentExecutionResult[],
  thresholds: GoNoGoThresholds
): Array<{ segment: string; confidence: EvidenceStrength }> {
  const segments = [
    ...new Set(results.map((r) => `${r.taskType}/${r.complexity}/${r.executionMode}`)),
  ];
  return segments.map((segment) => {
    const count = results.filter(
      (r) => `${r.taskType}/${r.complexity}/${r.executionMode}` === segment
    ).length;
...
function determineFinalVerdict(
  ...
  if (generalistDecisions.length === 0) {
    return {
      class: 'INCONCLUSIVE — Mais dados necessários',
      summary: 'No execution data available',
      productionDefault: 'unknown',
      premiumEscalation: 'unknown',
      blocked: [],
    };
  }
  ...
  const bestGeneralist = generalistDecisions
    .sort((a, b) => b.metrics.avgQuality - a.metrics.avgQuality)
    .find((d) => d.verdict === 'GO' || d.verdict === 'CONDITIONAL-GO');
  const productionDefault = bestGeneralist?.approach ?? 'single-model (best available)';

  // Premium escalation
  const maxQualityGo = decisions
    .filter(
      (d) => d.profile === 'max-quality' && (d.verdict === 'GO' || d.verdict === 'CONDITIONAL-GO')
    )
    .sort((a, b) => b.metrics.avgQuality - a.metrics.avgQuality)[0];
  const premiumEscalation = maxQualityGo?.approach ?? productionDefault;

  // Blocked approaches
  const blocked = [...new Set(noGoDecisions.map((d) => d.approach))].filter(
    (a) => generalistDecisions.filter((d) => d.approach === a && d.verdict === 'NO-GO').length > 0
  );
...
function answerMandatoryQuestions(
  ...
  const collectiveTier1Q = mean(
    results.filter((r) => r.executionMode === 'collective-tier1').map((r) => r.qualityScore!)
  );
  const budgetQ = mean(
    results.filter((r) => r.executionMode === 'single-budget').map((r) => r.qualityScore!)
  );
  ...
  const taskScores = (mode: string): TaskScore[] =>
    results
      .filter((r) => r.executionMode === mode && r.qualityScore != null)
  ...
  const pairedTier1Test =
    collectiveTier1Deltas.length >= 2 ? pairedTTest(collectiveTier1Deltas) : null;
  ...
  const goApproaches = [
    ...new Set(
      decisions
        .filter((d) => d.verdict === 'GO' && d.profile === 'generalist')
        .map((d) => d.approach)
    ),
  ];
  const noGoApproaches = [
    ...new Set(
      decisions
        .filter((d) => d.verdict === 'NO-GO' && d.profile === 'generalist')
        .map((d) => d.approach)
    ),
  ];
  const inconclusiveApproaches = [
    ...new Set(
      decisions
        .filter((d) => d.verdict === 'INCONCLUSIVE' && d.profile === 'generalist')
        .map((d) => d.approach)
    ),
  ];
  ...
  return {
    ...
    q2_collectiveBeatsTier1:
      pairedTest?.significant && pairedGain > 0
    ...
    q3_collectiveWinsWhere:
      collectiveWins.length > 0 ? collectiveWins : ['No scenarios identified'],
    q4_collectiveNotWorth:
      collectiveNotWorth.length > 0
        ? collectiveNotWorth
        : ['All scenarios show collective advantage'],
    q5_adaptiveSuperior:
      adaptiveQ > singleQ && adaptiveQ > collectiveQ
        ? `Yes (quality: ${adaptiveQ.toFixed(4)})`
        : `No (quality: ${adaptiveQ.toFixed(4)})`,
    q6_collectiveJustifiesCost:
      pairedGain > thresholds.minQualityGainForCollective
        ? 'Quality gain (paired, per-task) exceeds threshold'
        : 'Quality gain (paired, per-task) does not justify additional cost',
...
function getApproaches(
  results: ExperimentExecutionResult[]
): Array<{ label: string; mode: ExecutionMode }> {
...
function getLabel(r: ExperimentExecutionResult): string {
  switch (r.executionMode) {
    ...
    case 'ablation':
      return `ablation:${r.strategy}:${r.ablationCondition ?? ''}`;
    default:
