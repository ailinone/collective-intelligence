 ...
 *
 * Covers both the current unified ai1sk_ format and backward compatibility
 * with pre-rename legacy ak_-rooted shapes (ak_live_/ak_test_/ak_local_/
 * bare ak_<random>) — see api/src/utils/api-key-format.ts.
 */
...
describe('ApiKeyValue', () => {
  describe('Generation', () => {
    it('should generate a key with the current ai1sk_ prefix', () => {
      const apiKey = ApiKeyValue.generate();

      expect(apiKey).toBeInstanceOf(ApiKeyValue);
      expect(apiKey.getValue()).toMatch(/^ai1sk_[A-Za-z0-9_-]{32,}$/);
    ...
    it('should generate prefix for indexing', () => {
      ...
      expect(prefix).toMatch(/^ai1sk_[A-Za-z0-9_-]+$/);
    });
  });

  describe('Validation', () => {
    it('should create from a valid ai1sk_ key', () => {
      const validKey = 'ai1sk_' + 'a'.repeat(40);
      const apiKey = ApiKeyValue.create(validKey);

      expect(apiKey.getValue()).toBe(validKey);
    });

    it('should create from a legacy ak_live_ key (backward compat)', () => {
    ...
    it('should create from a legacy ak_test_ key (backward compat)', () => {
    ...
    it('should create from a legacy bare ak_ key (backward compat)', () => {
      const validKey = 'ak_' + 'c'.repeat(40);
      const apiKey = ApiKeyValue.create(validKey);
    ...
    it('should reject too short key', () => {
      expect(() => ApiKeyValue.create('ai1sk_123')).toThrow('Invalid API key format');
    });

    it('should reject too long key', () => {
      const tooLong = 'ai1sk_' + 'a'.repeat(100);
      expect(() => ApiKeyValue.create(tooLong)).toThrow('Invalid API key format');
    });

    it('should reject key with invalid characters', () => {
      expect(() => ApiKeyValue.create('ai1sk_abc@def#ghi' + 'x'.repeat(30))).toThrow(
  ...
  describe('Masking (Security)', () => {
    it('should mask a new-format key for display', () => {
      const apiKey = ApiKeyValue.generate();
      const masked = apiKey.getMasked();

      expect(masked).toMatch(/^ai1sk_[A-Za-z0-9_-]{3}\*\*\*\.\.\.\*\*\*[A-Za-z0-9_-]{3}$/);
      expect(masked).not.toBe(apiKey.getValue());
      expect(masked.length).toBeLessThan(apiKey.getValue().length);
    });

    it('should mask a legacy ak_live_ key without exposing the random part', () => {
      const legacyKey = ApiKeyValue.create('ak_live_' + 'a'.repeat(40));
      const masked = legacyKey.getMasked();

      expect(masked).toMatch(/^ak_[A-Za-z0-9_-]{3}\*\*\*\.\.\.\*\*\*[A-Za-z0-9_-]{3}$/);
      expect(masked).not.toBe(legacyKey.getValue());
    });
    ...
    it('should mask toJSON()', () => {
      ...
      expect(json).toMatch(/^ai1sk_.*\*\*\*.*$/);
      expect(json).not.toBe(apiKey.getValue());
    });
  });

  describe('Environment (legacy display only)', () => {
    it('reports live for new-format keys (no environment concept anymore)', () => {
      expect(ApiKeyValue.generate().getEnvironment()).toBe('live');
    });

    it('still reports test for a legacy ak_test_ key', () => {
      const legacyKey = ApiKeyValue.create('ak_test_' + 'a'.repeat(40));
      expect(legacyKey.getEnvironment()).toBe('test');
    });

    it('still reports live for a legacy ak_live_ key', () => {
      const legacyKey = ApiKeyValue.create('ak_live_' + 'a'.repeat(40));
      expect(legacyKey.getEnvironment()).toBe('live');
    });
  });

  describe('Equality', () => {
    it('should be equal if same key', () => {
      const key = 'ai1sk_' + 'a'.repeat(40);
