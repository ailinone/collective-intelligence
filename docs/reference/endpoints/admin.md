...
Execute admin discovery scheduled.

Run scheduled discovery (called by cron job)
...
```bash
curl -X POST "https://api.ailin.one/v1/admin/discovery/scheduled" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/admin/discovery/scheduled", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/admin/discovery/scheduled",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
...
Retrieve admin discovery stats.
...
```bash
curl -X GET "https://api.ailin.one/v1/admin/discovery/stats" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/admin/discovery/stats", {
  method: "GET",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "GET",
    "https://api.ailin.one/v1/admin/discovery/stats",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
...
Execute admin discovery trigger.

Manually trigger model discovery from all providers
...
```bash
curl -X POST "https://api.ailin.one/v1/admin/discovery/trigger" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/admin/discovery/trigger", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/admin/discovery/trigger",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
...
```bash
curl -X DELETE "https://api.ailin.one/v1/admin/users/:id" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/admin/users/:id", {
  method: "DELETE",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "DELETE",
    "https://api.ailin.one/v1/admin/users/:id",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
