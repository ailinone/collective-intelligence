...
```bash
curl -X POST "https://api.ailin.one/v1/embeddings" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input":"string","model":"string","encoding_format":"float"}'
```

```ts
const response = await fetch("https://api.ailin.one/v1/embeddings", {
  method: "POST",
  headers: {
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/embeddings",
    headers={
...
Create or execute embeddings create.

Generate embeddings (ailin-cli compatible endpoint)
...
```bash
curl -X POST "https://api.ailin.one/v1/embeddings/create" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input":"string","model":"string","encoding_format":"float"}'
...
```ts
const response = await fetch("https://api.ailin.one/v1/embeddings/create", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/embeddings/create",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
        "Content-Type": "application/json",
