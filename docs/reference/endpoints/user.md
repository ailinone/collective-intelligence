...
```bash
curl -X GET "https://api.ailin.one/v1/user/profile" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/user/profile", {
  method: "GET",
  headers: {
...
response = requests.request(
    "GET",
    "https://api.ailin.one/v1/user/profile",
    headers={
...
```bash
curl -X PUT "https://api.ailin.one/v1/user/profile" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY" \
