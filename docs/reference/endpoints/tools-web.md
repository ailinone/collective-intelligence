...
| Status | Description |
|---|---|
| `200` | Web search completed successfully |
...
```bash
curl -X POST "https://api.ailin.one/v1/tools/web-search" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query":"string","max_results":10,"search_type":"general","search_depth":"basic","working_directory":"string"}'
```

```ts
const response = await fetch("https://api.ailin.one/v1/tools/web-search", {
  method: "POST",
  headers: {
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/tools/web-search",
    headers={
