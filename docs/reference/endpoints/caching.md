...
Retrieve caching contexts.

Retrieve caching contexts.

Enterprise contract notes:
- Purpose: exposes /v1/caching/contexts as a governed API capability inside the CI Fabric.
...
```bash
curl -X GET "https://api.ailin.one/v1/caching/contexts" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/caching/contexts", {
  method: "GET",
  headers: {
...
response = requests.request(
    "GET",
    "https://api.ailin.one/v1/caching/contexts",
    headers={
...
Create or execute caching contexts.
...
```bash
curl -X POST "https://api.ailin.one/v1/caching/contexts" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```
...
Delete caching contexts context id.

Delete caching contexts context id.

Enterprise contract notes:
- Purpose: exposes /v1/caching/contexts/{context_id} as a governed API capability inside the CI Fabric.
- Preconditions: requires valid authentication unless explicitly marked as public.
- Side-effects: may emit telemetry/audit signals and mutate artifacts depending on method semantics.
- Limits: subject to tenant quotas, payload constraints, and rate-limit policies.
- Observability: request and correlation identifiers are propagated for traceability.
...
```bash
curl -X DELETE "https://api.ailin.one/v1/caching/contexts/sample" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/caching/contexts/sample", {
  method: "DELETE",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "DELETE",
    "https://api.ailin.one/v1/caching/contexts/sample",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
...
Retrieve caching contexts context id.
...
```bash
curl -X GET "https://api.ailin.one/v1/caching/contexts/sample" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```
...
Create or execute caching contexts context id use.

Create or execute caching contexts context id use.

Enterprise contract notes:
- Purpose: exposes /v1/caching/contexts/{context_id}/use as a governed API capability inside the CI Fabric.
- Preconditions: requires valid authentication unless explicitly marked as public.
- Side-effects: may emit telemetry/audit signals and mutate artifacts depending on method semantics.
- Limits: subject to tenant quotas, payload constraints, and rate-limit policies.
- Observability: request and correlation identifiers are propagated for traceability.
...
```bash
curl -X POST "https://api.ailin.one/v1/caching/contexts/sample/use" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/caching/contexts/sample/use", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/caching/contexts/sample/use",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
