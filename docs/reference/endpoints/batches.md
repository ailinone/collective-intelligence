...
- Purpose: exposes /v1/batches as a governed API capability inside the CI Fabric.
...
```bash
curl -X GET "https://api.ailin.one/v1/batches" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/batches", {
  method: "GET",
  headers: {
...
response = requests.request(
    "GET",
    "https://api.ailin.one/v1/batches",
    headers={
...
Retrieve batches batch id.

Retrieve batches batch id.

Enterprise contract notes:
- Purpose: exposes /v1/batches/{batch_id} as a governed API capability inside the CI Fabric.
- Preconditions: requires valid authentication unless explicitly marked as public.
- Side-effects: may emit telemetry/audit signals and mutate artifacts depending on method semantics.
- Limits: subject to tenant quotas, payload constraints, and rate-limit policies.
- Observability: request and correlation identifiers are propagated for traceability.
...
```bash
curl -X GET "https://api.ailin.one/v1/batches/sample" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/batches/sample", {
  method: "GET",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "GET",
    "https://api.ailin.one/v1/batches/sample",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
...
Create or execute batches batch id cancel.

Create or execute batches batch id cancel.

Enterprise contract notes:
- Purpose: exposes /v1/batches/{batch_id}/cancel as a governed API capability inside the CI Fabric.
- Preconditions: requires valid authentication unless explicitly marked as public.
- Side-effects: may emit telemetry/audit signals and mutate artifacts depending on method semantics.
- Limits: subject to tenant quotas, payload constraints, and rate-limit policies.
- Observability: request and correlation identifiers are propagated for traceability.
...
```bash
curl -X POST "https://api.ailin.one/v1/batches/sample/cancel" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/batches/sample/cancel", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AILIN_TOKEN}`,
    "X-API-Key": process.env.AILIN_API_KEY || "",
...
response = requests.request(
    "POST",
    "https://api.ailin.one/v1/batches/sample/cancel",
    headers={
        "Authorization": f"Bearer {os.environ.get('AILIN_TOKEN', '')}",
        "X-API-Key": os.environ.get("AILIN_API_KEY", ""),
    },
