...
## GET `/console/api/v1/jwks`

### Purpose

Retrieve console JWKS public keys.
...
| Status | Description |
|---|---|
| `200` | JWKS retrieved successfully. |
...
```bash
curl -X GET "https://api.ailin.one/console/api/v1/jwks" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/console/api/v1/jwks", {
  method: "GET",
  headers: {
...
response = requests.request(
    "GET",
    "https://api.ailin.one/console/api/v1/jwks",
    headers={
