...
## DELETE `/v1/queue/status/{id}`

### Purpose

Delete queue status id.

Delete queue status id.

Enterprise contract notes:
- Purpose: exposes /v1/queue/status/{id} as a governed API capability inside the CI Fabric.
...
```bash
curl -X DELETE "https://api.ailin.one/v1/queue/status/sample" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```

```ts
const response = await fetch("https://api.ailin.one/v1/queue/status/sample", {
  method: "DELETE",
  headers: {
...
response = requests.request(
    "DELETE",
    "https://api.ailin.one/v1/queue/status/sample",
    headers={
...
## GET `/v1/queue/status/{id}`

### Purpose

Retrieve queue status id.
...
```bash
curl -X GET "https://api.ailin.one/v1/queue/status/sample" \
  -H "Authorization: Bearer $AILIN_TOKEN" \
  -H "X-API-Key: $AILIN_API_KEY"
```
