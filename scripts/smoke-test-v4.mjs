...
/**
 * Smoke-test v4 — adds admin-token routing and full resource harvesting.
 *
 * Differences vs v3:
 *  - Three credential bags (USER_JWT / USER_API_KEY / ADMIN_KEY); per-route picker
 *    routes admin/users/{id}/organizations/{id} traffic through the admin key.
 *  - harvestPass now creates: file, response, thread, vector_store, assistant.
 *    file_id and response_id flow back into batches and follow-up calls.
 *  - jina/segment payload corrected (`input`, not `content`).
 *  - codebase/analysis sends a real `files` array (1 file, ~25 bytes) so the
 *    server doesn't 422 on empty input.
 *
 * Env:
 *   AILIN_BASE        default https://api.ailin.one — public client routes only
 *   AILIN_ADMIN_BASE   default http://localhost:3000 — internal target for
 *                       /v1/admin/*, /v1/users*, /v1/organizations/* (isAdminPath()
 *                       below). Admin routes are not part of the public contract
 *                       (2026-07-14) and won't resolve via AILIN_BASE.
 *   AILIN_JWT         user JWT (Bearer)
 *   AILIN_API_KEY     user API key (x-api-key)
 *   AILIN_ADMIN_KEY   admin/superuser API key (x-api-key, sent on admin paths)
 *   THROTTLE_MS       ms between request *starts* (default 1200)
 *   OUT               output file (default smoke-test-v4-results.json)
 */
...
const ADMIN_BASE = process.env.AILIN_ADMIN_BASE || 'http://localhost:3000';
const JWT = process.env.AILIN_JWT || null;
const API_KEY = process.env.AILIN_API_KEY || null;
const ADMIN_KEY = process.env.AILIN_ADMIN_KEY || null;
const CONCURRENCY = Number(process.env.CONCURRENCY || 1);
const OUT = process.env.OUT || './smoke-test-v4-results.json';
...
if (!ADMIN_KEY) {
  console.warn('WARN: AILIN_ADMIN_KEY not set — admin/users/organizations routes will likely return 401/403.');
}
...
// Routes that must use the admin key (if available)
function isAdminPath(p) {
  if (p.startsWith('/v1/admin/')) return true;
  if (p === '/v1/users' || p.startsWith('/v1/users/')) return true;
  if (/^\/v1\/organizations\/[^/]+/.test(p)) return true;
  return false;
}
...
const BODY_OVERRIDES = {
  ...
  // v4 fix: jina/segment expects `input`, not `content`
  'POST /v1/tools/jina/segment': { input: 'sentence one. sentence two.' },
  ...
  // v4 fix: codebase/analysis sends a real file so the server doesn't 422 on []
  ...
  // batches: file_id will be substituted from harvested.file_id at probe time
  'POST /v1/batches': { input_file_id: '__FILE_ID__', endpoint: '/v1/chat/completions', completion_window: '24h' },
...
// Apply harvested IDs into body templates after they are filled
function applyHarvestToBody(body) {
  if (!body || typeof body !== 'object') return body;
  if (Array.isArray(body)) return body.map(applyHarvestToBody);
  const out = {};
  for (const [k, v] of Object.entries(body)) {
    if (v === '__FILE_ID__') out[k] = harvested.file_id;
    else if (v === '__RESPONSE_ID__') out[k] = harvested.response_id;
    else if (v && typeof v === 'object') out[k] = applyHarvestToBody(v);
    else out[k] = v;
  }
  return out;
}
...
function synthValueLocal(schema, key, spec) {
  ...
  if (schema.$ref) return synthValueLocal(resolveRef(spec, schema.$ref), key, spec);
...
function synthBodyForKey(key, op) {
  if (BODY_OVERRIDES[key]) return applyHarvestToBody(BODY_OVERRIDES[key]);
  const scraped = routeSchemasNorm[key];
  if (scraped?.schema?.body) return synthValueLocal(scraped.schema.body, '', spec);
...
function synthQueryStringForKey(key, op) {
  ...
  if (scraped?.schema?.querystring) {
    ...
    for (const name of req) out[name] = synthValueLocal(props[name] || {}, name, spec);
  }
...
// Picks the right credential based on path. Admin paths prefer ADMIN_KEY; if
// missing, falls back to the regular auth so we still see what status we get.
function buildAuthHeaders(p) {
  const headers = {};
  if (isAdminPath(p) && ADMIN_KEY) {
...
async function probe(opEntry) {
  ...
  if (skip) return { method, path: p, status: -1, ms: 0, skipped: true, reason: 'skip-list' };

  const baseHeaders = buildAuthHeaders(p);
  const subbedPath = substituteParams(p);
  const qs = synthQueryStringForKey(key, op);
  const qsString = Object.keys(qs).length ? '?' + new URLSearchParams(qs).toString() : '';
  const url = `${isAdminPath(p) ? ADMIN_BASE : BASE}${subbedPath}${qsString}`;
  ...
  while (attempts <= MAX_RETRIES_429) {
    ...
    try {
      ...
      lastResult = {
        ...
        usedAdminKey: headers['x-api-key'] === ADMIN_KEY && ADMIN_KEY !== null,
        qs: qsString,
...
async function harvestPass() {
  ...
  // Order matters: file first (used by batches), then OpenAI-style entities,
  // then `responses` (which doesn't depend on anything but is useful for
  // /v1/responses/{id} GET probes).
  const creates = [
    { key: 'POST /v1/files',         save: (j) => harvested.file_id = j?.id || j?.file?.id || harvested.file_id },
    ...
    { key: 'POST /v1/responses',     save: (j) => harvested.response_id = j?.id || harvested.response_id },
  ];
  for (const c of creates) {
    ...
    if (!op) {
      console.log(`  ✗ ${c.key} → not in spec`);
      continue;
    }
    const r = await probe({ method: m, path: p, key: c.key, op, skip: false });
    if (r.status >= 200 && r.status < 300) {
      const j = r._harvest;
      const before = JSON.stringify(harvested);
      c.save(j);
      const after = JSON.stringify(harvested);
      if (before !== after) {
        console.log(`  ✓ ${c.key} → harvested`);
      } else {
        console.log(`  ✗ ${c.key} → 2xx but no usable id`);
      }
    } else {
      console.log(`  ✗ ${c.key} → ${r.status} ${r.snippet?.slice(0, 80) || ''}`);
    }
  }
  console.log('  harvested:', JSON.stringify({
    file_id: harvested.file_id,
    thread_id: harvested.thread_id,
    vector_store_id: harvested.vector_store_id,
    assistant_id: harvested.assistant_id,
    response_id: harvested.response_id,
  }));
}
...
async function runAll(ops) {
  ...
  async function worker() {
    while (i < ops.length) {
      ...
      const adminMark = r.usedAdminKey ? ' [admin]' : '';
      process.stdout.write(`[${String(idx + 1).padStart(3)}/${ops.length}] ${docFlag} ${tag.padEnd(4)} ${ops[idx].method.padEnd(6)} ${ops[idx].path}${t}${att}${adminMark}\n`);
    }
  }
...
console.log(`Auth: JWT=${!!JWT} API_KEY=${!!API_KEY} ADMIN_KEY=${!!ADMIN_KEY}`);
